...
def workflow_http_writes_dynamodb(
    api_name: str, route: str, request_json: dict, table_name: str, key: dict
) -> dict:
    try:
        ...
        if response.status_code < 200 or response.status_code >= 300:
            return {
                ...
                "passed": False,
                "message": f"Workflow HTTP response status {response.status_code} did not indicate success: {response.text[:200]}",
            }
        ...
        if item:
            return {
                ...
                "type": "workflow_http_writes_dynamodb",
                "passed": True,
                "message": f"Workflow wrote the expected item to {table_name}.",
            }
        return {
            "id": "http-writes-item",
            "type": "workflow_http_writes_dynamodb",
            "passed": False,
            "message": f"Workflow did not write the expected item to {table_name}. (Check if the Lambda function is correctly writing to DynamoDB)",
        }
        ...
        for message in messages:
            if expected_body_contains in message.get("Body", ""):
                return {
                    ...
                    "type": "workflow_http_sends_sqs",
                    "passed": True,
                    "message": f"Workflow sent the expected message to {queue_name}.",
                }
        return {
            "id": "http-sends-message",
            "type": "workflow_http_sends_sqs",
            "passed": False,
            "message": f"Workflow did not send the expected message to {queue_name}. (Check if the Lambda function is correctly sending to SQS)",
        }
    except Exception as e:
        return {
            "id": "http-sends-message",
