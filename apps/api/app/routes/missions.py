...
def list_missions(session: Session = Depends(get_session)):
    ...
    progress_map = {}
    results = session.query(MissionProgress).filter(MissionProgress.profile_id == "local").all()
    for p in results:
...
def get_mission(mission_id: str, session: Session = Depends(get_session)):
    ...
    completed_set = set(
        p.mission_id for p in session.query(MissionProgress).filter(
            MissionProgress.profile_id == "local",
    ...
    return {
        "mission": {
            ...
            "progress": {
                ...
                "completedAt": str(prog.completed_at) if prog and prog.completed_at else None,
            },
        }
    }
...
def start_mission(mission_id: str, session: Session = Depends(get_session)):
    ...
    mission = instances[mission_id]
    result = progress_service.start_mission(session, mission_id, mission.xp, mission.prerequisites)
    if "error" in result:
...
def reset_mission(mission_id: str, session: Session = Depends(get_session)):
    ...
    mission = instances[mission_id]
    result = progress_service.reset_mission(session, mission_id, mode, mission.prerequisites)
    if "error" in result:
        raise HTTPException(status_code=409, detail=result)
    return result
